...
func CreateJob(entityId int64, name string, created time.Time, earliestStart time.Time, cmd string, env, tag string) (int64, error) {
	res, err := db.Exec("INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, NULL, 0)", entityId, name, StatusCreated, created.Unix(), earliestStart.Unix(), cmd, env, tag)
	if err != nil {
		return 0, err
	}
...
func FindJobs(entityId int64) ([]Job, error) {
	rows, err := db.Query("SELECT id, entityId, name, status, created, earliestStart, started, ended, cmd, tag, runner, exitCode FROM jobs WHERE entityId = ? ORDER BY created ASC", entityId)
	if err != nil {
		return nil, err
	}
	results := []Job{}
...
func FindPrecedingJobs(id int64) ([]Job, error) {
	rows, err := db.Query("SELECT jobs.id, jobs.entityId, jobs.name, jobs.status, jobs.created, jobs.earliestStart, jobs.started, jobs.ended, jobs.cmd, jobs.tag, jobs.runner, jobs.exitCode FROM precedingJobs INNER JOIN jobs ON precedingJobs.olderJob = jobs.id WHERE precedingJobs.newerJob = ?", id)
	if err != nil {
		return nil, err
	}
	results := []Job{}
...
func FindQueuedJobs(before int64, limit int64) ([]Job, error) {
	query := "SELECT id, entityId, name, status, created, earliestStart, started, ended, cmd, tag, runner, exitCode FROM jobs WHERE status = ? "
...
func LoadJob(id int64) (Job, error) {
	rows, err := db.Query("SELECT id, entityId, name, status, created, earliestStart, started, ended, cmd, tag, runner, exitCode FROM jobs WHERE id = ?", id)
	if err != nil {
		return Job{}, err
	}
	if rows.Next() {
...
func ScanJob(rows *sql.Rows) (Job, error) {
	...
	var exitCode int64
	err := rows.Scan(&id, &entityId, &name, &statusInt, &createdTimestamp, &earliestStartTimestamp, &startedTimestamp, &endedTimestamp, &cmd, &tag, &runnerId, &exitCode)
	if err != nil {
		return Job{}, err
	}
	status := 0
	...
	return Job{Id: id, EntityId: entityId, Name: name, Status: status, Created: created, EarliestStart: earliestStart, Started: started, Ended: ended, Cmd: cmd, Tag: tag, Runner: runner, ExitCode: exitCode}, nil
}
...
func FillDatabaseWithDemoData() error {
	...
	for i := 1; i <= 135; i++ {
		...
		tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, 'build', ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 0)", i, StatusSucceeded, dt.Unix(), dt.Unix(), dt.Add(30*time.Second).Unix(), dt.Add(4*time.Minute).Unix())
	}
	...
	for i := 0; i < 10; i++ {
		...
		tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, 'build', ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 0)", 136+i, StatusSucceeded, dt.Unix(), dt.Unix(), dt.Add(30*time.Second).Unix(), dt.Add(630*time.Second).Unix())
	}
	for i := 0; i < 11; i++ {
		...
		tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, 'build', ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 0)", 147+i, StatusSucceeded, dt.Unix(), dt.Unix(), dt.Add(30*time.Second).Unix(), dt.Add(630*time.Second).Unix())
	}
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 0)", 146, "prepare", StatusSucceeded, t.Add(-134*time.Minute).Unix(), t.Add(-134*time.Minute).Unix(), t.Add(-133*time.Minute).Unix(), t.Add(-132*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 1)", 146, "build:linux", StatusFailed, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), t.Add(-128*time.Minute).Unix(), t.Add(-118*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 1)", 146, "build:linux", StatusFailed, t.Add(-108*time.Minute).Unix(), t.Add(-108*time.Minute).Unix(), t.Add(-107*time.Minute).Unix(), t.Add(-97*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 1)", 146, "build:linux", StatusFailed, t.Add(-85*time.Minute).Unix(), t.Add(-85*time.Minute).Unix(), t.Add(-84*time.Minute).Unix(), t.Add(-74*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 1)", 146, "build:linux", StatusFailed, t.Add(-62*time.Minute).Unix(), t.Add(-62*time.Minute).Unix(), t.Add(-61*time.Minute).Unix(), t.Add(-51*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 1)", 146, "build:linux", StatusFailed, t.Add(-39*time.Minute).Unix(), t.Add(-39*time.Minute).Unix(), t.Add(-38*time.Minute).Unix(), t.Add(-28*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 0)", 146, "build:linux", StatusSucceeded, t.Add(-16*time.Minute).Unix(), t.Add(-16*time.Minute).Unix(), t.Add(-15*time.Minute).Unix(), t.Add(-5*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,windows', 1, 0)", 146, "build:windows", StatusSucceeded, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), t.Add(-119*time.Minute).Unix(), t.Add(-107*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', NULL, 0)", 146, "test:linux", StatusCancelled, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), t.Add(-117*time.Minute).Unix(), t.Add(-116*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', 2, 0)", 146, "test:linux", StatusStarted, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), t.Add(-4*time.Minute).Unix(), nil)
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,windows', 1, 0)", 146, "test:windows", StatusSucceeded, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), t.Add(-107*time.Minute).Unix(), t.Add(-88*time.Minute).Unix())
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,macos', NULL, 0)", 146, "build:macos", StatusCreated, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), nil, nil)
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,macos', NULL, 0)", 146, "test:macos", StatusCreated, t.Add(-132*time.Minute).Unix(), t.Add(-132*time.Minute).Unix(), nil, nil)
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', 'docker,linux', NULL, 0)", 146, "deploy", StatusCreated, t.Add(-132*time.Minute).Unix(), t.Add(348*time.Minute).Unix(), nil, nil)
