...
func FillDatabaseWithDemoData() error {
	...
	tryExec(tx, "INSERT INTO jobs (id, entityId, name, status, created, earliestStart, started, ended, auth, cmd, env, tag, runner, exitCode) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, NULL, '', '', 'docker,linux', 2, 0)", 146, "prepare", StatusSucceeded, t.Add(-134*time.Minute).Unix(), t.Add(-134*time.Minute).Unix(), t.Add(-133*time.Minute).Unix(), t.Add(-132*time.Minute).Unix())
